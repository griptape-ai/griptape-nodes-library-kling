...
class KlingAI_ImageToVideo(ControlNode):
    ...
    def _get_image_api_data_from_input(self, image_input) -> str | None:
        ...
        if isinstance(image_input, dict):
            ...
            if input_type == "ImageArtifact" and base64_from_dict:
    ...
    def after_incoming_connection_removed(
        ...
        if target_parameter.name in ["image", "image_tail", "static_mask"]:
            self.set_parameter_value(target_parameter.name, None)
            self.publish_update_to_parameter(target_parameter.name, None)
